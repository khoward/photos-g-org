...
Provides a beautiful glassmorphism web UI on port 9090.
"""
...
@require_api_key
def start_organize():
    """Start organizing photos with input validation."""
    ...
    # Validate year
    year_valid, year_error, year = validate_year(data.get('year'))
    if not year_valid:
    ...
    logger.info(f"Organization started by {get_client_ip()} for year {year}")

    # Start organization in background thread
    thread = threading.Thread(
        target=_run_organize,
        args=(year, album_id, album_name, skip_existing)
    )
    thread.daemon = True
    thread.start()
...
def _run_organize(year: int, album_id: str, album_name: str, skip_existing: bool):
    ...
    try:
        ...
        # Search for photos
        with organize_lock:
            organize_state['message'] = f'Searching for photos from {year}...'

        def search_progress(count):
            with organize_lock:
                organize_state['message'] = f'Found {count} photos from {year}...'
        ...
        if not photo_ids:
            with organize_lock:
                ...
                organize_state['message'] = f'No photos found for {year}'
            return
...
def run_server(host: str = '127.0.0.1', port: int = 9090, debug: bool = False, public: bool = False):
    """
